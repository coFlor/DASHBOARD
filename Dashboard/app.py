...
@st.cache_resource
def load_data():
    df = pd.read_csv("valencia.csv", index_col="tipo_habitación")
...
if show_sidebar:
    with st.sidebar:
        ...
        logo = Image.open("valencia.png")
        st.sidebar.image(logo, width=100)
        st.sidebar.header("Valencia, España")
...
if view == "Vista 1: Categóricas & Numéricas":
    ...
    st.markdown("Airbnb en Valencia · Variables Categóricas")

    logo = Image.open("vista_v.jpg")
    st.image(logo, width=300)

    st.markdown("Alojamientos de Airbnb en Valencia.")
    ...
    # Mapa
    st.title("Mapa de Alojamientos en Valencia")
    map_df = df.rename(columns={"latitud": "latitude", "longitud": "longitude"})

    map_df = map_df[
        (map_df['latitude'].between(39.3, 39.6)) & 
        (map_df['longitude'].between(-0.5, -0.2))
    ]
